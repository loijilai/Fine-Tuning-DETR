...
def infer(images_path, model, postprocessors, device, output_path):
    ...
    for img_sample in images_path:
        ...
        outputs["pred_logits"] = outputs["pred_logits"].cpu() # [9, 100, 1]
        outputs["pred_boxes"] = outputs["pred_boxes"].cpu() # [4, 100, 1]

        probas = outputs['pred_logits'].softmax(-1)[0, :, :-1] # [100, 8]
        # keep = probas.max(-1).values > 0.85
        keep = probas.max(-1).values > args.thresh # A T/F mask

        bboxes_scaled = rescale_bboxes(outputs['pred_boxes'][0, keep], orig_image.size) # [4, 5]
        ...
        plot_results(img, probas, bboxes_scaled)
